...
#ifdef CRAY
void GETS(line) char *line;
#else
#ifdef TITAN
void GETS(line) char **line;
#else
void gets_(line) char *line;
#endif
#endif
{
  int i;
...
#ifdef TITAN
void XPUTXT(x, y, fontsize, string, strlength) float *x, *y, *fontsize;
char **string;
int *strlength;
#else
#ifdef CRAY
...
int *strlength;
#else
void xputxt_(x, y, fontsize, string, strlength) float *x, *y, *fontsize;
char *string;
int *strlength;
#endif
#endif
...
#ifdef TITAN
void PUTS(message, nchars) char **message;
int *nchars;
#else
#ifdef CRAY
void PUTS(message, nchars) char *message;
int *nchars;
#else
void puts_(message, nchars) char *message;
int *nchars;
#endif
#endif
{
...
void backSpace()
{
  ...
  fountainPenPosition.x -= char_spacing;
  if (fountainPenPosition.x < 10)
    fountainPenPosition.x = 10;
  XDrawImageString(dpy, theWindow, fountainPenGC,
                   fountainPenPosition.x, fountainPenPosition.y, "_", 1);
  XFlush(dpy);
...
#ifdef CRAY
void SYSTEM(string) char *string;
{
  system(string);
}
...
void RENAME(from, to) char *from, *to;
{
  rename(from, to);
}
#endif
